  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Testimonials</h2>
      ...
      @if (loading()) {
        ...
        <app-empty-state iconName="lucideStar" title="No testimonials yet" message="Customer reviews will appear here." />
      } @else {
          ...
          @for (t of items(); track t.id) {
            <article class="card p-5 flex flex-col gap-3 hover:shadow-md transition-shadow" role="listitem">
              <div class="flex items-start justify-between gap-3">
                ...
                <div class="flex items-center gap-0.5 flex-shrink-0" [attr.aria-label]="t.rating + ' out of 5 stars'">
                  @for (i of [1,2,3,4,5]; track i) {
                    <ng-icon name="lucideStar" size="13" [class]="i <= t.rating ? 'text-amber-400' : 'text-gray-200 dark:text-gray-700'" />
                  }
                </div>
              </div>
              <blockquote class="text-sm text-gray-600 dark:text-gray-300 italic line-clamp-4 border-l-2 border-primary-200 dark:border-primary-700 pl-3">
                "{{ t.content }}"
              </blockquote>
            </article>
          }
...
export class TestimonialsComponent implements OnInit {
  ...
  avgRating(): number {
    ...
    return list.length ? list.reduce((s, t) => s + t.rating, 0) / list.length : 0;
  }
